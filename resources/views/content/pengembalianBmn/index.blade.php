  ...
  <div class="br-mainpanel">
    <div class="br-pageheader pd-y-15 pd-l-20">
      <nav class="breadcrumb pd-0 mg-0 tx-12">
        ...
        <span class="breadcrumb-item active">Pengembalian BMN</span>
      </nav>
    </div><!-- br-pageheader -->
    ...
    <div class="pd-x-20 pd-sm-x-30 pd-t-2 d-flex align-items-center justify-content-between flex-wrap">
      <div>
        <h4 class="tx-gray-800 mg-b-5">Pengembalian BMN</h4>
        <p class="mg-b-0">Pengajuan Pengembalian BMN</p>
      </div>
    </div>
    ...
    <div class="br-pagebody">
      <div class="br-section-wrapper">
        ...
        <div class="table-wrapper">
                ...
                <tr>
                  ...
                  <td>
                    <div class="mb-1">
                      ...
                      @endif
                      <a target="_blank" href="/peminjamanBmn/{{ $peminjamanBmn->id }}/pdf"
                        class="px-2 py-1 tx-12 bg-info text-light rounded"><i class="bi bi-filetype-pdf"></i> Pdf</a>
                    </div>
                    <span class="tx-12">{{ $peminjamanBmn->nomor }}</span>
                  </td>
                  <td class="text-center">
                    <a class="btn btn-teal tx-10 pd-x-10 pd-y-5 text-light tx-medium" data-toggle="modal"
                      data-target="#modaldemo{{ $peminjamanBmn->id }}">Detail BMN</a>
                    {{-- Modal BMN --}}
                                  ...
                                  <tr>
                                    ...
                                    @if (isset($bmn['kesediaan']))
                                      @if ($bmn['kesediaan'] == 'Tersedia')
                                        <td><i class="bi bi-patch-check-fill tx-success"></i> {{ $bmn['kesediaan'] }}
                                        </td>
                                      @else
                                        <td><i class="bi bi-patch-exclamation-fill tx-danger"></i>
                                          {{ $bmn['kesediaan'] }}</td>
                                      @endif
                                    @else
                  ...
                  <td>
                    {{ $peminjamanBmn->lama_pinjam }} Hari<br>
                    <span class="tx-12">{{ $peminjamanBmn->lokasi_penggunaan }}</span>
                  </td>
                  <td class="text-center">
                    @if ($peminjamanBmn->status == 'Disetujui')
                      <a class="btn btn-success tx-10 pd-x-10 pd-y-5 text-light tx-medium" data-toggle="modal"
                        data-target="#modalpengembalian{{ $peminjamanBmn->id }}"><i class="bi bi-clipboard"></i>
                        Pengembalian</a>
                    @elseif ($peminjamanBmn->status == 'Selesai')
                      <a class="btn btn-indigo tx-10 pd-x-10 pd-y-5 text-light tx-medium" data-toggle="modal"
                        data-target="#modalDataPengembalian{{ $peminjamanBmn->id }}"><i
                          class="bi bi-clipboard-check"></i>
                        Data Pengembalian</a>
                      {{-- Modal --}}
                      <div id="modalDataPengembalian{{ $peminjamanBmn->id }}" class="modal fade">
                        <div class="modal-dialog modal-sm w-100" role="document">
                          <div class="modal-content bd-0 tx-14">
                            <div class="modal-header pd-x-20">
                              <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">BMN</h6>
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                              </button>
                            </div>
                            <div class="modal-body pd-20">
                              ...
                              <hr>
                              <span class="tx-12 tx-teal">Nama : </span> <br>
                              {{ $peminjamanBmn->pengembalianBmn->nama }} <br>
                              <span class="tx-12 tx-teal">Tanggal Pengembalian : </span> <br>
                              {{ \Carbon\Carbon::parse($peminjamanBmn->pengembalianBmn->tanggal_pengembalian)->translatedFormat('d F Y') }}
                              <br>
                              <span class="tx-12 tx-teal">Kondisi Barang : </span> <br>
                              {{ $peminjamanBmn->pengembalianBmn->kondisi_barang }}
                            </div>
                            <div class="modal-footer justify-content-end">
                              <button type="button"
                                class="btn btn-secondary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium"
          ...
          @foreach ($peminjamanBmns as $peminjamanBmn)
            {{-- Modal --}}
            <div id="modalpengembalian{{ $peminjamanBmn->id }}" class="modal fade">
              <div class="modal-dialog modal-lg w-100" role="document">
                <form id="accept{{ $peminjamanBmn->id }}" action="/manage/pengembalianBmn" method="POST"
                  class="modal-content bd-0 tx-14">
                  @csrf
                  <div class="modal-header pd-x-20">
                    <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Pengembalian BMN</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                  <div class="modal-body pd-20">
                    <input type="hidden" name="peminjaman_bmn_id" value="{{ $peminjamanBmn->id }}">
                    <div class="form-group">
                      <label class="form-control-label">Nama <span class="tx-danger">*</span></label>
                      <input class="form-control" type="text" name="nama"
                        placeholder="Enter Nama Yang Menyerahkan" required>
                    </div>
                    <div class="form-group">
                      <label class="form-control-label">Tanggal Pengembalian <span class="tx-danger">*</span></label>
                      <input class="form-control" type="date" name="tanggal_pengembalian"
                        placeholder="Enter Tanggal Pengembalian" required>
                    </div>
                    <div class="form-group">
                      <label class="form-control-label">Kondisi Barang <span class="tx-danger">*</span></label>
                      <input class="form-control" type="text" name="kondisi_barang"
                        placeholder="Enter Kondisi Barang" required>
                    </div>
                  ...
                  <div class="modal-footer justify-content-end">
                    <button data-kode="{{ $peminjamanBmn->id }}" type="button"
                      class="btn btn-success tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium accepted-confirm">Submit</button>
                    <button type="button"
                      class="btn btn-secondary tx-11 tx-uppercase pd-y-12 pd-x-25 tx-mont tx-medium"
                      data-dismiss="modal">Close</button>
                  </div>
