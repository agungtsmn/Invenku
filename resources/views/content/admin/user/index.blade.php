...
@section('content')
  ...
  <div class="br-mainpanel">
    ...
    <div class="pd-x-20 pd-sm-x-30 pd-t-2 d-flex align-items-center justify-content-between flex-wrap">
      <div>
        <h4 class="tx-gray-800 mg-b-5">Management User</h4>
        <p class="mg-b-0">Menambah, Mengedit, dan Menghapus Data User</p>
      </div>
      <a href="/manage/user/create" class="btn btn-teal btn-create">Create Data <i class="bi bi-plus"></i></a>
    </div>
                ...
                <tr>
                  ...
                  <td>
                    <div class="d-flex justify-content-center">
                      <a href="/manage/user/{{ $user->id }}/edit" class="btn btn-warning tx-10 pd-x-10 pd-y-5 mr-2"><i class="bi bi-pen mr-1"></i> Edit</a>
                      @if ($user->id == 1)
                      @else
                        <div data-kode="{{ $user->id }}" class="btn btn-danger tx-10 pd-x-10 pd-y-5 swal-confirm">
                          <form action="/manage/user/{{ $user->id }}" id="delete{{ $user->id }}" method="post">
                              @csrf
                              @method('delete')
                          </form>
