...
<body>
  ...
  <section><br>
    <div class="text-center" tabindex="0">
      <p class="tx-16"><b>FORM PERMINTAAN ATK</b></p>
      <p class="tx-14">NOMOR: {{ $data->nomor ?? "-" }}</p>
    </div><br>
    <span>Tanggal: {{ \Carbon\Carbon::parse($data->tanggal)->translatedFormat('d F Y') }}</span><br>
    <span>Jenis: {{ $data->jenis }}</span><br>
    <span>Lokasi: {{ $data->lokasi }}</span><br><br>
    <table class="table-component">
      ...
      <tbody>
        @foreach ($data->atk as $atk)
          <tr>
            <td class="text-center tx-14">{{ $loop->iteration }}</td>
            <td class="tx-14">{{ $atk['nama'] ?? "-" }}</td>
